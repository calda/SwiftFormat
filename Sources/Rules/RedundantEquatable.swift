    ...
    static let redundantEquatable = FormatRule(
        ...
        for equatableType in typesManuallyImplementingEquatableConformance {
            ...
            // can be generated by that macro instead of written manually.
                ...
                modificationsByIndex[equatableType.equatableConformanceIndex] = {
                    formatter.removeConformance(at: equatableType.equatableConformanceIndex)
                }
...
extension Formatter {
    ...
    func addImports(_ importsToAddIfNeeded: Set<String>) {
        ...
        for importToAddIfNeeded in importsToAddIfNeeded {
            ...
            let insertionIndex = firstImport?.range.lowerBound ?? 0

            insert(
                [.keyword("import"), .space(" "), .identifier(importToAddIfNeeded), linebreakToken(for: insertionIndex)],
                at: insertionIndex
            )
        }
    }
